...
class TSP(object):
    def __init__(self, M: list, nombreArchivo, solInicial, nroIntercambios, opt, tenureADD, tenureDROP, tiempoEjec, optimo):
        ...
        self.__frecMatriz = []
        for _ in range(0, len(self._G.getMatriz())):
            fila = []
            for _ in range(0, len(self._G.getMatriz())):
                fila.append(0)
    ...
    #Elimina los vertices de la lista que no tengan Tenure -1
    def eliminaListaTabu(self, lista_tabu):
        lista_tabu_aux=[]

        for x in lista_tabu:
            if(x.getTenure() == -1):
                lista_tabu_aux.append(x)
        self.borraFrecuentados(lista_tabu)
        
        return lista_tabu_aux
        ...
        while(tiempoEjecuc <= tiempoMax):
            ...
            if(len(lista_permit)>=4):
                ...
                if(time()-tiempoIniEstancamiento > tiempoMaxNoMejora):    
                    ...
                    porcentaje = round(Sol_Optima.getCostoAsociado()/self.__optimo -1.0, 3)*100
                    if(tiempoRestante <= 3*60 and porcentaje > 11.0):
                        print("El porcentaje de desviacion es muy alto. Eliminamos algunos vertices")
                        lista_tabu = self.eliminaListaTabu(lista_tabu)
                ...
                self.eliminaListaTabu(lista_tabu)
